...
type Options struct {
	...
	GetInfoBurst      uint64
	TCPGlobalPerSec   uint64 // aggregate cap across CPUs; 0 disables
	TCPGlobalBurst    uint64 // aggregate burst across CPUs
	TCPMaxOpenPerIP   uint64 // 0 disables
...
func loadXDP(opts Options) (*fivemXDPObjects, error) {
	...
	// Per-CPU split for the global TCP circuit breaker. Aggregate cap stays
	// what the user configured; each CPU's bucket is 1/ncpu of it.
