...
// -1 if the pin can't be opened. Iterates the whole map — fine for status
// output, expensive at scale; don't call on a tight loop.
...
// blacklistedNow returns the count of udp_health entries currently blocked
// (blacklist_until_ns > now). Returns -1 if the map can't be read.
