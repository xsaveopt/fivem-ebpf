...
// cmdHealth iterates udp_health. By default lists only currently-blocked
// IPs (the actionable subset). With --all, also includes IPs being tracked
// for anomalies but not yet over the threshold — useful for spotting
// bots ramping up before they trip the limiter.
func cmdHealth(args []string) {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "open udp_health:", err)
		os.Exit(1)
	}
	defer m.Close()
