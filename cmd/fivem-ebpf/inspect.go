...
// cmdInspect prints everything the daemon currently knows about a single
// IPv4 across every per-IP map. It opens the pinned maps directly (same
// pattern as dump/stats/clear) so it doesn't need the metrics endpoint
// or HTTP — works as long as the daemon has the pins live.
